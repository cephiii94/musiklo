// app.js - Versi Final dengan Sticky Header & Expanded Player
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // BARU: Seleksi elemen untuk Expanded Player
    ...
    // === 2. STATE APLIKASI (Tidak ada perubahan) ===
    let player;
    let currentPlaylist = [];
    let currentIndex = -1;
    let isPlaying = false;
    ...
    // === 3. INISIALISASI YOUTUBE PLAYER (Tidak ada perubahan) ===
    ...
    // === 4. MEMUAT DAN MENAMPILKAN LAGU (Tidak ada perubahan) ===
    ...
    // === 5. LOGIKA PEMUTARAN (Tidak ada perubahan) ===
    ...
    // === 6. UPDATE PROGRESS & VOLUME (Tidak ada perubahan) ===
    ...
    // === 7. FUNGSI PENCARIAN & KEMBALI KE HOME (Tidak ada perubahan) ===
    ...
    // === 8. EVENT LISTENERS ===
    songListContainer.addEventListener('click', (event) => { const songItem = event.target.closest('.song-item'); if (songItem) { const index = parseInt(songItem.dataset.index, 10); playSong(index); } });
    ...
    // BARU: Event listener untuk membuka dan menutup Expanded Player
    currentTrackArt.addEventListener('click', () => {
        if (currentIndex !== -1) { // Hanya buka jika ada lagu yang sedang aktif
